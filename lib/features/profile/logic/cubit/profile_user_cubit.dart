...
class ProfileUserCubit extends Cubit<ProfileUserState> {
  ...
  Future updateProfile() async {
    ...
    final response = await _profileUserRepo.updateProfile(
    ...
    response.when(success: (profileUserMode) {
      emit(
        ProfileUserState.successUpdate(profileUserMode: profileUserMode),
      );
    }, failure: (error) {
      emit(ProfileUserState.errorUpdste(
          error: error.apiErrorModel.message ?? ' '));
    });
  }
  // void uploadProfileImage(XFile? image) {
  //   updateImage = image;
  // emit(const ProfileUserState.pickImage());
  //
  // }
}
